...
def clone_or_update_docs(docs_src: Path = DEFAULT_DOCS_SRC, force_clone: bool = False) -> Path:
...
def main():
    ...
    parser.add_argument(
        "--force-clone",
        action="store_true",
        help="Force fresh clone of docs repo"
    )
    parser.add_argument(
        "--skip-clone",
        action="store_true",
        help="Skip cloning, use existing docs"
    )
    parser.add_argument(
        "--keep-existing",
        action="store_true",
        help="Keep existing index entries"
    )
